...
import { getAINewsAnalysis } from '@/service/news';
import { type AINewsAnalysisParam, type AINewsAnalysisRequest } from '@/interfaces/Dto/News';

export type AnalysisProps = AINewsAnalysisParam & AINewsAnalysisRequest;

function Analysis({ id, summary }: AnalysisProps) {
  /* 
  ...
  */
  const { data } = useQuery(['news', 'analysis', id, summary], () => getAINewsAnalysis({ id, summary }), {
    retry: 0,
  ...
  return (
    <div>
      ...
      <div className="mt-1 box-border flex w-full items-center justify-center bg-orange-400 py-1 text-sm text-white">
        오늘 남은 횟수:&nbsp;{data?.leftBenefitCount}회
      </div>
    </div>
  );
}
